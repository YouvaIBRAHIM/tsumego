...
export const shapeMarkers = (
  ...
  for (let k in markers) {
    ...
    if ("cross" == markers[k]) {
      ...
      cls =
        markers[k] + " on" + (positions[k] || positions[other(k, size)] || "white")
      ...
      const x3: number =
        x + (step / SV_MARKER) * Math.cos(Math.PI / 2 + (4 * Math.PI) / 3)
      const y3: number =
        y - (step / SV_MARKER) * Math.sin(Math.PI / 2 + (4 * Math.PI) / 3)
