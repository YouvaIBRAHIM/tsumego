...
const schema: yup.ObjectSchema<IProblemCreate> = yup.object({
  ...
  level: yup
    .mixed<IProblemCreate["level"]>()
    .oneOf(Object.values(LevelsEnum))
    .required(),
  ...
  nextToPlay: yup
    .mixed<IProblemCreate["nextToPlay"]>()
    .oneOf(Object.values(NexToPlayEnum))
    .required(),
  ...
  const submit: SubmitHandler<IProblemCreate> = async (formData) => {
    ...
    if (status !== 201) {
      setStateModal((prev) => ({
        ...prev,
        open: true,
        message: <ErrorResponse error={res.label[0]} />,
      }))
      return
    }
    closeModal()
        ...
        <form id="tsumegoForm" onSubmit={handleSubmit(submit)}>
          ...
          >
            <InputLabel
              ...
              sx={{ color: `${theme.palette.text.disabled} !important` }}
            >
              Niveau du Tsumego
            </InputLabel>
            <Select
              ...
              {...register("level")}
              sx={{
                ".MuiSelect-outlined": { borderColor: "red" },
  ...
  return (
    <>
      {isEmpty ? (
        <>
          <Typography>
            Il semblerait que vous n'ayez pas encore posé de pierres...{" "}
          </Typography>
          <Typography>
            Pour proposer un Tsumego il faut au moins {MIN_STONES} pierres.
          </Typography>
        </>
      ) : (
